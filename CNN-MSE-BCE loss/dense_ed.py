...
class DenseEDTI(nn.Module):
    def __init__(self, in_channels, out_channels, blocks, growth_rate=16,
                 num_init_features=64, bn_size=4, drop_rate=0, bottleneck=False):
        """
        Args:
    ...
    def forward(self, x, times):
        # x: (N, 1, iH, iW)
        # times: (N,)
        times = times.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand_as(x)
        x = F.sigmoid(self.features(torch.cat((x, times), 1)))
        return x
