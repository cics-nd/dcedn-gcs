...
class Parser(argparse.ArgumentParser):
    def __init__(self):
        ...
        # default to use cuda
        self.add_argument('--exp-name', type=str, default='DCEDN_binary', help='experiment name')
        ...
        self.add_argument('--data-dir', type=str, default="/afs/crc.nd.edu/user/s/smo/co2UQ/DCEDN_binary/data/epsilon0.0", help='data directory')
        ...
        self.add_argument('-lc', '--lambda-class', type=float, default=0.01, help='coefficient of classification loss')
        ...
        self.add_argument('--log-interval', type=int, default=10, help='how many epochs to wait before logging training status')
        self.add_argument('--plot-interval', type=int, default=50, help='how many epochs to wait before plotting training status')

    def parse(self):
        args = self.parse_args()

        # Normalizing the time in the network, 200 is the max simulation time instance
        args.times = np.array(args.times) / 200
        print(args.times)

        # output directory path
        args.exp_dir = '/afs/crc.nd.edu/user/s/smo/co2UQ/{}/'.format(args.exp_name) + "experiments_2step_1-200/ntrn{}_blocks{}_batch{}_epochs{}_lr{}_wd{}_lc{}_t{}_K{}".\
            format(args.n_train, args.blocks, args.batch_size, args.n_epochs,
                   args.lr, args.weight_decay, args.lambda_class, len(args.times),args.growth_rate)
